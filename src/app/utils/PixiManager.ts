...
export default class PixiManager {
  ...
  public init(el: HTMLElement, _width: number, _height: number, backgroundColor: number):void{
    ...
    this.app = new PIXI.Application({width: width, height: height, backgroundColor: backgroundColor, resolution: 1, autoStart: true});
  ...
  public createPolygon(points:Array<{x:number,y:number}>, props:object, graphics:any = null) {
    ...
    let noGraphics = graphics === undefined || graphics === null
    let g = noGraphics ? new PIXI.Graphics() : graphics
    if (noGraphics) {
      g.clear()
    }
    g.x = properties.x;
